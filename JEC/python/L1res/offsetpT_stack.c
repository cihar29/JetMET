...
void offsetpT_stack( TString mcName="histoMC_R4.root", TString dataName="histoData_R4.root", TString bin_var="nPU", Id id=all, bool ratio=true,
  ...
  int max_var = h_bin_var->GetBinCenter( h_bin_var->GetMaximumBin() );
  //int max_var = h_bin_var->GetMean();
  ...
  for (int i=0; i<numId; i++) {
    ...
    hname = Form("p_offset_eta_%s%i_", bin_var.Data(), max_var) + ids[i];
    ...
    v_MC[i]->Scale( 1.0 / max_var );
    v_Data[i]->Scale( 1.0 / max_var );
  }
  ...
  if (id == all){
    ...
    h1->GetYaxis()->SetRangeUser( 0, dataStack->GetMaximum()*1.7 );
    mcStack->Draw("samehist");
    dataStack->Draw("samepe");
    cloneStack->Draw("samepe");

    TLegend* leg = new TLegend(.37,.6,.67,.9);
    leg->SetBorderSize(0);
    leg->SetFillColor(0);
    leg->SetFillStyle(0);
    ...
    if (ratio) {
      ...
      h2->GetYaxis()->SetRangeUser(0.8, chs_Data->GetMaximum()*1.1 );
    ...
    h1->GetYaxis()->SetRangeUser( 0, hist_Data->GetMaximum()*1.4 );
  ...
  c->Print("stack_" + ids[id] + "_" + bin_var + to_string(max_var) + ".pdf");
}
